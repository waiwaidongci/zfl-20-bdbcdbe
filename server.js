...
const { readFile, writeFile, mkdir, unlink, rename, copyFile } = require("fs/promises");
...
async function handle(req, res) {
  ...
  if (backupValidateMatch && req.method === "GET") {
    const filename = backupManager.sanitizeBackupFilename(decodeURIComponent(backupValidateMatch[1]));
    try {
  ...
  if (backupDiffMatch && req.method === "GET") {
    try {
      const filename = backupManager.sanitizeBackupFilename(decodeURIComponent(backupDiffMatch[1]));
      const result = await backupManager.computeBackupDiff(filename);
      if (result.error) {
        return send(res, 400, {
          data: result,
  ...
  if (backupRestoreMatch && req.method === "POST") {
    const filename = backupManager.sanitizeBackupFilename(decodeURIComponent(backupRestoreMatch[1]));
    try {
      const body = await parseBody(req);
      ...
      let backupInfo;
      try {
        const backupValidation = await backupManager.validateBackup(filename);
        backupInfo = {
      ...
      try {
        precomputedDiff = await backupManager.computeBackupDiff(filename);
      } catch (diffError) {
        precomputedDiff = null;
      }

      const result = await backupManager.restoreFromBackup(filename, precomputedDiff, expectedVersion);
